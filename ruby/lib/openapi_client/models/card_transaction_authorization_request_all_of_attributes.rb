...
module OpenapiClient
  class CardTransactionAuthorizationRequestAllOfAttributes
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::CardTransactionAuthorizationRequestAllOfAttributes` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::CardTransactionAuthorizationRequestAllOfAttributes`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [created_at, amount, status, partial_approval_allowed, approved_amount, decline_reason, merchant, recurring, tags, payment_method, digital_wallet, card_verification_data, card_network].hash
    end
