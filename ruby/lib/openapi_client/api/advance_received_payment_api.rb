...
module OpenapiClient
  class AdvanceReceivedPaymentApi
    ...
    def execute_with_http_info(payment_id, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: AdvanceReceivedPaymentApi.execute ...'
      end
      # verify the required parameter 'payment_id' is set
      if @api_client.config.client_side_validation && payment_id.nil?
        fail ArgumentError, "Missing the required parameter 'payment_id' when calling AdvanceReceivedPaymentApi.execute"
      end
      # resource path
      ...
      new_options = opts.merge(
        :operation => :"AdvanceReceivedPaymentApi.execute",
      ...
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: AdvanceReceivedPaymentApi#execute\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
