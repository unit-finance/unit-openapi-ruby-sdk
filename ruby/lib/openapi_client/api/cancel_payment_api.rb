  ...
  class CancelPaymentApi
    ...
    def execute_with_http_info(payment_id, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: CancelPaymentApi.execute ...'
      end
      # verify the required parameter 'payment_id' is set
      if @api_client.config.client_side_validation && payment_id.nil?
        fail ArgumentError, "Missing the required parameter 'payment_id' when calling CancelPaymentApi.execute"
      end
      # resource path
      ...
      new_options = opts.merge(
        :operation => :"CancelPaymentApi.execute",
      ...
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: CancelPaymentApi#execute\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
