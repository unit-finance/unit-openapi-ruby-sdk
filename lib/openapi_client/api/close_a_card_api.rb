  ...
  class CloseACardApi
    ...
    def execute_with_http_info(card_id, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: CloseACardApi.execute ...'
      end
      # verify the required parameter 'card_id' is set
      if @api_client.config.client_side_validation && card_id.nil?
        fail ArgumentError, "Missing the required parameter 'card_id' when calling CloseACardApi.execute"
      end
      # resource path
      ...
      new_options = opts.merge(
        :operation => :"CloseACardApi.execute",
      ...
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: CloseACardApi#execute\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
