...
module OpenapiClient
  class GetCardPINStatusApi
    ...
    # @param card_id [String] ID of the card to get PIN status
    # @param [Hash] opts the optional parameters
    # @return [Array<(UnitCardResponse2, Integer, Hash)>] UnitCardResponse2 data, response status code and response headers
    def execute_with_http_info(card_id, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: GetCardPINStatusApi.execute ...'
      end
      # verify the required parameter 'card_id' is set
      if @api_client.config.client_side_validation && card_id.nil?
        fail ArgumentError, "Missing the required parameter 'card_id' when calling GetCardPINStatusApi.execute"
      end
      # resource path
      ...
      # return_type
      return_type = opts[:debug_return_type] || 'UnitCardResponse2'

      # auth_names
      auth_names = opts[:debug_auth_names] || ['bearerAuth']

      new_options = opts.merge(
        :operation => :"GetCardPINStatusApi.execute",
      ...
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: GetCardPINStatusApi#execute\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
