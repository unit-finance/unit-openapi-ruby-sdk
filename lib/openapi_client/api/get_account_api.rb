  ...
  class GetAccountApi
    ...
    def execute_with_http_info(account_id, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: GetAccountApi.execute ...'
      end
      # verify the required parameter 'account_id' is set
      if @api_client.config.client_side_validation && account_id.nil?
        fail ArgumentError, "Missing the required parameter 'account_id' when calling GetAccountApi.execute"
      end
      # resource path
      ...
      new_options = opts.merge(
        :operation => :"GetAccountApi.execute",
      ...
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: GetAccountApi#execute\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
