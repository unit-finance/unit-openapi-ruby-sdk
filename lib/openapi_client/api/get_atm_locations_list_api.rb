...
module OpenapiClient
  class GetATMLocationsListApi
    ...
    # @option opts [ExecuteFilterParameter17] :filter 
    ...
    def execute_with_http_info(opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: GetATMLocationsListApi.execute ...'
      end
      # resource path
      ...
      new_options = opts.merge(
        :operation => :"GetATMLocationsListApi.execute",
      ...
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: GetATMLocationsListApi#execute\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
