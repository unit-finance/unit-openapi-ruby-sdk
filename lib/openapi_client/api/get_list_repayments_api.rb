...
module OpenapiClient
  class GetListRepaymentsApi
    ...
    # @option opts [ExecuteFilterParameter20] :filter 
    ...
    def execute_with_http_info(opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: GetListRepaymentsApi.execute ...'
      end
      # resource path
      ...
      new_options = opts.merge(
        :operation => :"GetListRepaymentsApi.execute",
      ...
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: GetListRepaymentsApi#execute\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
