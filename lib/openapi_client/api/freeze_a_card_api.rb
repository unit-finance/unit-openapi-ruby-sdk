  ...
  class FreezeACardApi
    ...
    def execute_with_http_info(card_id, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: FreezeACardApi.execute ...'
      end
      # verify the required parameter 'card_id' is set
      if @api_client.config.client_side_validation && card_id.nil?
        fail ArgumentError, "Missing the required parameter 'card_id' when calling FreezeACardApi.execute"
      end
      # resource path
      ...
      new_options = opts.merge(
        :operation => :"FreezeACardApi.execute",
      ...
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: FreezeACardApi#execute\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
