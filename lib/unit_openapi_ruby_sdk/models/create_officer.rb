  ...
  class CreateOfficer
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `UnitOpenapiRubySdk::CreateOfficer` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `UnitOpenapiRubySdk::CreateOfficer`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @occupation.nil?
        invalid_properties.push('invalid value for "occupation", occupation cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @occupation.nil?
      true
    end
    ...
    def hash
      [full_name, email, phone, ssn, passport, nationality, matricula_consular, address, date_of_birth, title, evaluation_params, occupation, annual_income, source_of_income].hash
    end
