...
module UnitOpenapiRubySdk
  class IndividualApplicationAllOfAttributes
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `UnitOpenapiRubySdk::IndividualApplicationAllOfAttributes` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `UnitOpenapiRubySdk::IndividualApplicationAllOfAttributes`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [created_at, updated_at, status, message, evaluation_outcome, evaluation_id, evaluation_entity_id, full_name, email, phone, ssn, passport, nationality, matricula_consular, address, date_of_birth, dba, ein, ip, sole_proprietorship, decision_method, decision_user_id, decision_reason, decision_date_time, tags, risk_rate, evaluation_flags, evaluation_scores, ip_location_details, phone_location_details, masked_ssn, masked_passport, masked_matricula_consular, jwt_subject, archived, power_of_attorney_agent, industry, id_theft_score, occupation, annual_income, source_of_income, annual_revenue, number_of_employees, business_vertical, website].hash
    end
