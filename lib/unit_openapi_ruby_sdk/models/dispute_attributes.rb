  ...
  class DisputeAttributes
    ...
    def self.openapi_types
      {
        ...
        :'amount' => :'Integer',
        :'status' => :'String',
        :'status_history' => :'Array<DisputeAttributesStatusHistoryInner>',
        :'created_at' => :'Time',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `UnitOpenapiRubySdk::DisputeAttributes` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `UnitOpenapiRubySdk::DisputeAttributes`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      source_validator = EnumAttributeValidator.new('String', ["DebitCard", "ACH"])
      return false unless source_validator.valid?(@source)
      return false if @external_id.nil?
      return false if @external_id.to_s.length > 255
      return false if !@link.nil? && @link !~ Regexp.new(/(http(s)?:\/\/.)?(www\.)?[-a-zA-Z0-9@:%._\+~#=]{2,256}\.[a-z]{2,6}\b([-a-zA-Z0-9@:%_\+.~#?&\/\/=]*)/)
    ...
    def source=(source)
      validator = EnumAttributeValidator.new('String', ["DebitCard", "ACH"])
    ...
    def hash
      [source, external_id, link, description, amount, status, status_history, created_at, decision_reason].hash
    end
