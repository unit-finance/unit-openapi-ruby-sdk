...
module UnitOpenapiRubySdk
  class CreateCheckStopPaymentAttributes
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `UnitOpenapiRubySdk::CreateCheckStopPaymentAttributes` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `UnitOpenapiRubySdk::CreateCheckStopPaymentAttributes`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @check_number.nil?
        invalid_properties.push('invalid value for "check_number", check_number cannot be nil.')
      end

      if @check_number.to_s.length > 12
        invalid_properties.push('invalid value for "check_number", the character length must be smaller than or equal to 12.')
      end
    ...
    def valid?
      ...
      return false if @check_number.nil?
      return false if @check_number.to_s.length > 12
    ...
    def check_number=(check_number)
      if check_number.nil?
        fail ArgumentError, 'check_number cannot be nil'
      end

      if check_number.to_s.length > 12
        fail ArgumentError, 'invalid value for "check_number", the character length must be smaller than or equal to 12.'
      end
    ...
    def hash
      [amount, check_number, tags, idempotency_key].hash
    end
