  ...
  class ApplicationFormPrefill
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `UnitOpenapiRubySdk::ApplicationFormPrefill` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `UnitOpenapiRubySdk::ApplicationFormPrefill`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [application_type, full_name, ssn, passport, nationality, date_of_birth, email, name, state_of_incorporation, entity_type, contact, officer, beneficial_owners, website, phone, address, dba, ein, jwt_subject, industry, business_vertical, occupation].hash
    end
