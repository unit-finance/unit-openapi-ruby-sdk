  ...
  class Bin
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `UnitOpenapiRubySdk::Bin` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `UnitOpenapiRubySdk::Bin`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      pattern = Regexp.new(/^\d{9}$/)
      if @institution_id !~ pattern
        invalid_properties.push("invalid value for \"institution_id\", must conform to the pattern #{pattern}.")
      end
    ...
    def valid?
      ...
      return false if @bin !~ Regexp.new(/^\d{9}$/)
      return false if @institution_id.nil?
      return false if @institution_id !~ Regexp.new(/^\d{9}$/)
      true
    end
    ...
    def institution_id=(institution_id)
      ...
      pattern = Regexp.new(/^\d{9}$/)
      if institution_id !~ pattern
        fail ArgumentError, "invalid value for \"institution_id\", must conform to the pattern #{pattern}."
      end
