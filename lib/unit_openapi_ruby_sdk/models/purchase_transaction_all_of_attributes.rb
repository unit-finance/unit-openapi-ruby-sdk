...
module UnitOpenapiRubySdk
  class PurchaseTransactionAllOfAttributes
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `UnitOpenapiRubySdk::PurchaseTransactionAllOfAttributes` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `UnitOpenapiRubySdk::PurchaseTransactionAllOfAttributes`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @ecommerce.nil?
        invalid_properties.push('invalid value for "ecommerce", ecommerce cannot be nil.')
      end

      if @card_present.nil?
        invalid_properties.push('invalid value for "card_present", card_present cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @ecommerce.nil?
      return false if @card_present.nil?
      true
    end
    ...
    def hash
      [created_at, direction, amount, balance, summary, card_last4_digits, merchant, coordinates, recurring, tags, network_transaction_id, interchange, ecommerce, card_present, international_service_fee, payment_method, digital_wallet, card_verification_data, card_network].hash
    end
