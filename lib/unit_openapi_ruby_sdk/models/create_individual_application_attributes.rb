...
module UnitOpenapiRubySdk
  class CreateIndividualApplicationAttributes
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `UnitOpenapiRubySdk::CreateIndividualApplicationAttributes` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `UnitOpenapiRubySdk::CreateIndividualApplicationAttributes`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [full_name, email, phone, ssn, passport, nationality, address, date_of_birth, ip, tags, idempotency_key, device_fingerprints, jwt_subject, power_of_attorney_agent, industry, evaluation_params, occupation, annual_income, source_of_income].hash
    end
