  ...
  class PowerOfAttorneyAgent
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `UnitOpenapiRubySdk::PowerOfAttorneyAgent` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `UnitOpenapiRubySdk::PowerOfAttorneyAgent`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [status, full_name, email, phone, ssn, passport, nationality, matricula_consular, address, date_of_birth, evaluation_id, evaluation_flags, masked_ssn, masked_passport, masked_matricula_consular, id_theft_score].hash
    end
