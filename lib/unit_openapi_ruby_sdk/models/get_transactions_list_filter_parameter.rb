...
module UnitOpenapiRubySdk
  class GetTransactionsListFilterParameter
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `UnitOpenapiRubySdk::GetTransactionsListFilterParameter` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `UnitOpenapiRubySdk::GetTransactionsListFilterParameter`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'exclude_fees')
        self.exclude_fees = attributes[:'exclude_fees']
      end
    ...
    def hash
      [account_id, account_ids, customer_id, query, tags, since, _until, card_id, exclude_fees, type, direction, from_amount, to_amount, account_type].hash
    end
