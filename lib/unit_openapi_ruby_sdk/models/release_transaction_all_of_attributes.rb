...
module UnitOpenapiRubySdk
  class ReleaseTransactionAllOfAttributes
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `UnitOpenapiRubySdk::ReleaseTransactionAllOfAttributes` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `UnitOpenapiRubySdk::ReleaseTransactionAllOfAttributes`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@sender_account_number.nil? && @sender_account_number.to_s.length > 17
        invalid_properties.push('invalid value for "sender_account_number", the character length must be smaller than or equal to 17.')
      end
    ...
    def valid?
      ...
      return false if !@sender_account_number.nil? && @sender_account_number.to_s.length > 17
    ...
    def sender_account_number=(sender_account_number)
      if sender_account_number.nil?
        fail ArgumentError, 'sender_account_number cannot be nil'
      end

      if sender_account_number.to_s.length > 17
        fail ArgumentError, 'invalid value for "sender_account_number", the character length must be smaller than or equal to 17.'
      end
    ...
    def hash
      [created_at, direction, amount, balance, summary, sender_name, description, sender_account_number, sender_address, counterparty, tags].hash
    end
