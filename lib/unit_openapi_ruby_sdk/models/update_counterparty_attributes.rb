...
module UnitOpenapiRubySdk
  class UpdateCounterpartyAttributes
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `UnitOpenapiRubySdk::UpdateCounterpartyAttributes` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `UnitOpenapiRubySdk::UpdateCounterpartyAttributes`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@plaid_processor_token.nil? && @plaid_processor_token.to_s.length > 60
        invalid_properties.push('invalid value for "plaid_processor_token", the character length must be smaller than or equal to 60.')
      end

      if !@plaid_processor_token.nil? && @plaid_processor_token.to_s.length < 40
      ...
      if !@plaid_processor_token.nil? && @plaid_processor_token !~ pattern
    ...
    def valid?
      ...
      return false if !@plaid_processor_token.nil? && @plaid_processor_token.to_s.length > 60
      return false if !@plaid_processor_token.nil? && @plaid_processor_token.to_s.length < 40
      return false if !@plaid_processor_token.nil? && @plaid_processor_token !~ Regexp.new(/^processor-.*$/)
    ...
    def hash
      [plaid_processor_token, name, permissions, verify_name, tags].hash
    end
